...
void DX::DeviceResources::CreateDeviceResources() 
{
	...
	HRESULT hr = D3D11CreateDevice(
		...
		D3D_DRIVER_TYPE_HARDWARE,	// 创建使用硬件图形驱动程的设备。
	...
	// 将指针存储到 Direct3D 11.1 API 设备和即时上下文。
	DX::ThrowIfFailed(
...
// 注册我们的 DeviceNotify 以便在设备丢失或创建时收到通知。
...
void DX::DeviceResources::Present() 
{
	...
	// 在下一个 VSync 前进入休眠。这将确保我们不会浪费任何周期渲染将
	// 从不会在屏幕上显示的帧。
